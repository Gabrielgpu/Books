...
def tocar_alerta():
    pygame.mixer.init()
    pygame.mixer.music.load(r"C:\Users\User\Desktop\Registrar livros up\database\repetidos.mp3")
...
def inserir_produto(isbn):
    ...
    try:
        cursor.execute("SELECT numero FROM removidos.dados_up ORDER BY numero ASC LIMIT 1")
        numero_removido = cursor.fetchone()

        if numero_removido:
            tocar_alerta()
            numero_usado = numero_removido[0]
            cursor.execute("DELETE FROM removidos.dados_up WHERE numero = %s", (numero_usado,))
        else:
            
            cursor.execute("SELECT MAX(CAST(numero AS UNSIGNED)) FROM produtos_up")
        ...
        cursor.execute("INSERT INTO produtos_up (numero, isbn_modificado, isbn) VALUES (%s, %s, %s)", (numero_usado, isbn_modificado, isbn))
        db.commit()

        print(f"Produto inserido na tabela de produtos_up com número {numero_usado} e ISBN {isbn}.")
...
def remover_produto_por_isbn(isbn_modificado):
    ...
    try:
        if len(str(isbn_modificado)) > 10:
            query_select = "SELECT numero, isbn FROM produtos_up WHERE isbn_modificado = %s"
            query_delete = "DELETE FROM produtos_up WHERE isbn_modificado = %s"
        else:
            query_select = "SELECT numero, isbn FROM produtos_up WHERE numero = %s"
            query_delete = "DELETE FROM produtos_up WHERE numero = %s"
        ...
        if resultado:
            ...
            cursor.execute("SELECT numero FROM removidos.dados_up WHERE numero = %s", (numero,))
            ...
            if not numero_existente:
                ...
                if cursor.rowcount > 0:
                    ...
                    cursor.execute("INSERT INTO removidos.dados_up (numero) VALUES (%s)", (numero,))
                    db.commit()
